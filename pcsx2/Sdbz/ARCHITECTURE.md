...
- **2026-09-26, M1 done (FUC).** FUC runs on `GameRollback` from `manifests/fuc.yaml`, generated by `tools/gen_fuc_manifest.py`, with zero code caves and zero game-memory patches.
  - **Resim driver:** re-simulation is a host state machine. A hook on the sim-tick call site runs each manifest step by setting pc to the game function, with `$ra` pointing at a dead-code address that is itself hooked. The original call runs last. No EE thunk code is needed.
  - **Hooks used:**
    - resim gates, including gates that set `v0`;
    - call-site skips, both resim-only and always;
    - entry and post-call hooks;
    - entry actions (memory ops, then return or continue);
    - native `$ra`-filtered call hooks for the sound-only RNG sites;
    - pad feed as an entry hook plus a post-call hook.
  - **Results:** 8f sync test on every frame gives 0 desync. The A/B against the cave backend is within noise over 5 interleaved pairs. Forward and 8f runs are identical by in-round frame for every stream start and SE request.
  - **Harness:** `load_setup` applies all of a run's setup on the state-load frame, so runs are frame-exact repeats.
- **M2 in progress.** Manifests are YAML (rapidyaml is already in PCSX2), not TOML. Added:
  - address expressions, with `u8/u16/u32[..]`, arithmetic and comparisons, a loop index `i`, and one-parameter macros such as SDBZ `ph(x)`;
  - `*_file` data lists;
  - region ends read through expressions;
  - `rollback.gate_when` phase predicates;
